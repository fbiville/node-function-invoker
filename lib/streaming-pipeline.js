...
module.exports = class StreamingPipeline extends Writable {
    ...
    _wireOutputs(outputContentTypes) {
        ...
        for (let i = 0; i < outputCount; i++) {
            const outputMarshaller = new OutputMarshaller(i, outputContentTypes[i], this._options);
            outputMarshaller.pipe(this._destinationStream, {end: false});
            outputMarshaller.on('error', (err) => {
                logger('Error from marshaller');
                this.emit('error', err);
            });
            outputMarshaller.on('finish', () => {
            ...
            this._functionOutputs[i] = outputMarshaller;
        }
    }
