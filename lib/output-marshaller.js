...
const sensitiveLogger = require('util').debuglog('riff-sensitive');
...
module.exports = class OutputMarshaller extends Transform {
    ...
    _transform(value, _, callback) {
        ...
        sensitiveLogger(`Raw payload ${message.payload.toString()} for output #${this._index} (content-type: ${this._acceptedContentType})`);
        let payload;
        try {
        ...
        sensitiveLogger(`Payload after marshalling: ${payload} (output #${this._index})`);
        sensitiveLogger(`Headers: ${message.headers} (output #${this._index})`);
